...
if (isset($_SESSION['type'])) {
  if($_SESSION['type']=='C') {
    echo <<<HEREDOC
    <div>
    <button class="btn-hover" onclick = "addToCart({$_GET["pID"]},{$_GET["ID"]},'{$_GET['name']}',{$_GET['price']},'{$_GET['image']}','{$_GET['des']}',getQuantity({$_GET["pID"]}))" ><i onclick="popUp()">Add to Cart</i> </button>
    </div>
